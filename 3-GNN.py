...
class ThreeGNN(torch.nn.Module):
    def __init__(self):
        ...
        self.conv6 = GraphConv(num_i_3, WIDTH).cuda()
        self.conv_layers = torch.nn.ModuleList()
        for layer in range(LAYERS):
            self.conv_layers.append(GraphConv(WIDTH, WIDTH).cuda())

        self.fc1 = torch.nn.Linear(WIDTH, WIDTH).cuda()
        self.fc2 = torch.nn.Linear(WIDTH, 32).cuda()
        self.fc3 = torch.nn.Linear(32, dataset.num_classes).cuda()
