...
class Net(torch.nn.Module):
    def __init__(self):
        ...
        for layer in range(LAYERS):
            self.conv_layers.append(GraphConv(WIDTH, WIDTH, norm=NORM).cuda()) 
    ...
    def forward(self, data):
        if int(RANDOM_RATIO * WIDTH) > 0:  # Randomness Exists
            random_dims = torch.empty(data.x.shape[0], int(RANDOM_RATIO * WIDTH)).cuda()  # Random INIT
            if DISTRIBUTION == "n":
